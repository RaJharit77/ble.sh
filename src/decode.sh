  ...
  if
    if ((key==_ble_decode_IsolatedESC)); then
      if [[ $2 == "_$_ble_decode_IsolatedESC" ]]; then
        ...
        # "ble-bind -k".  In this case, we translate it to 27.
        key=27
      fi
    else
      ...
      while ((${#arg})); do
        ...
        (k)
          ...
          if [[ ${optarg[1]} && ${optarg[1]} != - ]]; then
            ...
            if [[ $kc == *' '* ]]; then
              ble/util/print "ble-bind: -k: the target key (${optarg[1]}) cannot contain multiple keys." >&2
              flags=E$flags
              continue
            fi
...
function ble/builtin/bind/rlfunc2widget/load-dict {
  ...
  local script='
    if ((!${#NAME[@]})); then
      ...
      ble/base/is-msys && NAME=("${NAME[@]%$_ble_term_cr}") # disable=#D2352
    fi
    dict=("${NAME[@]}")
